...
> {
  ...
  // Association methods for B2BAppUser
      ...
      },
      {
        ...
        indexes: [
          ...
          },
          {
            name: 'usr_uk1',
            unique: true,
            // This matches your actual database constraint
            fields: [
              { name: 'customer_id' },
              // For case-insensitive username uniqueness per customer
              {
                name: 'username',
                // Sequelize syntax for functional index with UPPER()
                // Note: This might need raw SQL depending on Sequelize version
              },
            ],
          },
        ],
        ...
        hooks: {
          beforeCreate: async (user) => {
            if (user.password) {
              if (!AppUser.isPasswordComplex(user.password)) {
                throw new Error(
          ...
          beforeUpdate: async (user) => {
            // if (!B2BAppUser.isPasswordComplex(user.password)) {
